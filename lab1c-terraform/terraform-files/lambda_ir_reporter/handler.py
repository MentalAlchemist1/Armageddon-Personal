...
def lambda_handler(event, context):
    now = int(time.time())
    end_ts = now
    start_ts = now - 60 * 60  # 15 min “fast report” window
    ...
    app_errors = run_insights_query(
        APP_LOG_GROUP,
        "fields @timestamp, @message | filter @message like /ERROR|DB|timeout|refused|Access denied/ | sort @timestamp desc | limit 50",
        start_ts, end_ts
    )
    app_rate = run_insights_query(
        APP_LOG_GROUP,
        "fields @timestamp, @message | filter @message like /ERROR|DB|timeout|refused|Access denied/ | stats count() as errors by bin(1m) | sort 'bin(1m)' asc",
        start_ts, end_ts
    )
